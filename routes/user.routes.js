...
const { Signup } = require('../middleware/validationSchemas')
...
router.get('/listUsers', async (req, res) => {
  ...
  return res.status(StatusCodes.ACCEPTED).json({ users: users })
})
router.delete('/deleteuser/', async (req, res) => {
  const id = req.query.id
  await deleteUserById(id)
  return res.status(StatusCodes.ACCEPTED).json({ message: 'User deleted' })
})
